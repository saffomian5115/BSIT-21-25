...
router.put('/profile', protect, async (req, res) => {
  try {
    ...
    await user.save();
    const updated = await User.findById(req.user._id).select('-password -resetPasswordToken -resetPasswordExpire');
    res.json(updated);
  } catch (error) { res.status(500).json({ message: error.message }); }
});
...
router.get('/dashboard', protect, async (req, res) => {
  try {
    ...
    res.json({ user, stats: { totalChats: chats.length, totalMessages, recentChats: chats.slice(-3).reverse() } });
  } catch (error) { res.status(500).json({ message: error.message }); }
});
